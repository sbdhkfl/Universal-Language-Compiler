...
def parse(text: str) -> Program:
    ...
    if m:
        ...
        p.add("if_print", name=m.group(1), op=ops[m.group(3).lower()], compare=_literal(m.group(4)), value=_literal(m.group(5)))
        return p

    m = re.fullmatch(r"(?:repeat|loop)\s+(\d+)\s+times\s+(?:print|display|say)\s+(.+)", text, re.I)
    if m:
        p.add("repeat_print", count=int(m.group(1)), value=_literal(m.group(2)))
        return p
    ...
    if m:
        ...
        for item in m.group(2).split(","):
            ...
            if len(parts) < 2:
                raise TranslationError("SQL columns must include a name and type, e.g. id integer.")
            columns.append((parts[0], " ".join(parts[1:])))
        p.add("create_table", name=m.group(1), columns=columns)
        return p

    raise TranslationError("I do not recognize that instruction yet. Try a documented command pattern.")
